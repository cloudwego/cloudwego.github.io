...
3. The semantic of `stream.Close()` on the client side is `CloseSend`, informing the server that there will be no new messages (the server `Recv` call returns `io.EOF`), and does not close the receiving end, so it cannot be used as the cancel method.
