...
hz is a command line tool provided by the Hertz framework for generating code. Currently, hz can generate scaffolding for Hertz projects based on thrift and protobuf's IDL.

## Install

1. Make sure the `GOPATH` environment variable has been defined correctly (eg `export GOPATH=~/go`) and add `$GOPATH/bin` to the PATH environment (eg `export PATH=$GOPATH/bin:$PATH`); do not set a directory in which you have no read and write permission as `GOPATH`.
2. Install hz:
...
**Note**，Since hz creates soft links to its own binaries, make sure that the installation path of hz has writable permissions.
...
The code generated by hz, part of it is generated by the underlying compiler (usually about the structure defined in IDL), and the other part is the user-defined routing, method and other information in IDL. The user can run the code directly.

In terms of execution flow, when hz uses thrift IDL to generate code, hz calls thriftgo to generate the go structure code and executes itself as a plug-in to thriftgo (named thrift-gen-hertz) to generate the rest of the code. This is also true when used with the protobuf IDL.
...
When executed, it generates a scaffold for the Hertz project in the current directory.
...
If it returns `{"message":"pong"}`, it means that the interface is connected.
...
If it returns `{"RespBody":"hello,hertz"}`, it means that the interface is connected.
...
If it returns `{"Resp":"Other method: other method"}`, it means that the interface is connected.
...
```bash
// Execute under GOPATH, if the dependencies of the main IDL and the main IDL are not in the same path, you need to add the -I option, its meaning is IDL search path, equivalent to protoc's -I command
...
│   │   │   └── example
│   │   │       ├── hello_service.go   // the handler file, the user implements the method defined by the IDL service in this file, and will search for the existing handler in the current file when updating, and append a new handler to the end
│   │   │       └── new_service.go     // same as above, each searvice defined in IDL corresponds to a file
│   │   └── ping.go                    // ping handler carried by defaul, used to generate code for quick debugging, no other special meaning
│   ├── model                          // IDL content-related generation code
│   │   └── hello                      // hello/example corresponds to the namespace defined in thrift IDL; for protobuf IDL, it corresponds to the last level of go_package
│   │     └── example
│   │         └── hello.go             // the product of thriftgo, the go code containing the contents of hello.thrift definition, will be regenerated on update
│   └── router                         // routing related generated code defined in IDL
│       ├── hello                      // hello/example corresponds to the namespace defined in thrift IDL; for protobuf IDL, it corresponds to the last level of go_package
│       │   └── example
...
├── idl                                // use defined IDL, location can be arbitrary
│   └── hello.thrift
...
>
> Method annotation can be used to generate route registration related code
...
The path generated by model will be:
...
// In this case, hz will not update the model code under "biz/model", but upder "my_model"; then the code under "biz/model" and "my_model" will be duplicated, and the new handler will depend on "my_model",while the previous handler will depend on "biz/model". In this case, you need to delete & change some code manually.
```
