...
- **传输协议（Transport Protocol）**：定义了消息如何在网络中传输，包括头部信息、元数据等，如 TTHeader、HTTP2 等
...
- **Protobuf 的两种使用模式**：Kitex 对 Protobuf 的支持有 Kitex Protobuf 和 gRPC 两种模式
  ...
  - **gRPC**：完整的 gRPC 协议栈，支持 Thrift 和 Protobuf 作为消息协议，HTTP2 作为传输协议。
  - **为什么将 gRPC 视为传输协议**：虽然 gRPC 实际上是一个完整的 RPC 框架，包含了消息协议(Protobuf)和传输协议(HTTP2)，但在 Kitex 的配置体系中，我们将 gRPC 归类为传输协议选项，使用户能够通过简单的 `WithTransportProtocol(transport.GRPC)` 一次性启用完整的 gRPC 协议栈，而不需要分别配置消息协议和 HTTP2
...
##### 2. gRPC 模式
当需要与 gRPC 服务互通时，需要显式启用 gRPC 协议（HTTP2 + Protobuf）：

```go
var opts []client.Option
opts = append(opts, client.WithTransportProtocol(transport.GRPC))
opts = append(opts, client.WithMetaHandler(transmeta.ClientHTTP2Handler)) // HTTP2 元数据处理
cli := xxx.NewClient("service_name", opts...)
```
...
```
- Kitex **会自动启用 gRPC 协议**（因为流式 API 必须依赖 HTTP2）
- 无需额外配置
...
```go
var opts []server.Option
opts = append(opts, server.WithMetaHandler(transmeta.ServerHTTP2Handler)) // HTTP2 元数据处理
svr, err := xxx.NewServer(handler, opts...)
```
