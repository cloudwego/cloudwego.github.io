...
对于 Server 来说，默认使用 netpoll，可以通过配置项进行更改，也可以通过设置 `HERTZ_NO_NETPOLL=true` 来禁用 netpoll：

> 注意：netpoll 仅支持 linux/darwin && amd64/arm64 平台，目前不支持 Windows，Windows 会通过条件编译将网络库自动切换为 go net。此外，TLS 场景会回退到标准网络库。
...
对于 Client 来说，可以通过配置项进行更改（也可以通过设置 `HERTZ_NO_NETPOLL=true` 来禁用 netpoll）：
